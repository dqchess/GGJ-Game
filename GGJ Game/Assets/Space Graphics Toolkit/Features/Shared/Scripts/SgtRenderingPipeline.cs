	...
	public class SgtRenderingPipeline : ScriptableObject
	{
		public static bool IsScriptable
		{
			get
			{
				if (instance == null)
				{
					instance = Resources.Load<SgtRenderingPipeline>("SgtRenderingPipeline");
				}
		...
		private void Apply(bool srp)
		{
			...
			var shaders  = AssetDatabase.FindAssets("t:Shader").Select(g => AssetDatabase.LoadAssetAtPath<Shader>(AssetDatabase.GUIDToAssetPath(g))).Where(s => s.name.Contains("SGT"));
...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtRenderingPipeline_Editor : Editor
	{
		...
		{
			var Target = (SgtRenderingPipeline)target;
