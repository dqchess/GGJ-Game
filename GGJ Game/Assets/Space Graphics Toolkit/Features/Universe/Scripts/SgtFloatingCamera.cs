...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtFloatingCamera : SgtLinkedBehaviour<SgtFloatingCamera>
	{
		...
		public static void TrySnapAll()
		{
			var floatingCamera = FirstInstance;

			for (var i = 0; i < InstanceCount; i++)
			{
				floatingCamera.TrySnap();
		...
		private static void CheckForPositionChangesAll()
		{
			...
			for (var i = 0; i < InstanceCount; i++)
			{
				if (floatingCamera.UseOrigin == true)
				{
					floatingCamera.CheckForPositionChanges();
				}
