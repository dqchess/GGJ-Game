...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtFloatingWarpPin : MonoBehaviour
	{
		...
		{
			if (SgtFloatingCamera.InstanceCount > 0)
			{
		...
		protected virtual void LateUpdate()
		{
			...
			if (SgtFloatingCamera.InstanceCount > 0)
			{
				var camera = SgtFloatingCamera.FirstInstance;

				if (CurrentTarget != null)
				{
					var localPosition = camera.CalculatePosition(ref CurrentTarget.CachedPoint.Position);
					var screenPoint   = camera.CachedCamera.WorldToScreenPoint(localPosition);
